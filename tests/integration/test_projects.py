def test_create_project_success(client, create_user):
    ...
    response = client.post(
        "/api/v1/projects",
        json={"name": "E-Commerce App", "description": "API backend for shop", "status": "active"},
        headers=mgr["headers"],
    )
...
def test_create_project_invalid_data(client, create_user):
    ...
    response = client.post(
        "/api/v1/projects", json={"description": "Missing required fields"}, headers=mgr["headers"]
    )
...
def test_get_projects_list_forbidden_for_regular_user(client, create_user):
    ...
    response = client.get("/api/v1/projects", headers=normal_user["headers"])
...
def test_update_project_not_found(client, create_user):
    ...
    response = client.patch(
        "/api/v1/projects/99999", json={"name": "Updated Name"}, headers=admin["headers"]
    )
