...
def create_authenticated_user(client, email: str, role: str = "user", firstname: str = "Test"):
    ...
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": email, "password": "SecurePassword123!"}
    )
...
def test_api1_bola_user_a_cannot_access_user_b_project(client, create_user):
    ...
    proj_resp = client.post(
        "/api/v1/projects",
        json={"name": "User B Project", "description": "Confidential", "status": "active"},
    ...
    access_resp = client.get(f"/api/v1/projects/{project_id}", headers=user_a["headers"])
...
def test_api1_bola_user_a_cannot_access_user_b_task_or_cross_project(client, create_user):
    ...
    p1_resp = client.post(
        "/api/v1/projects",
        json={"name": "Project Alpha", "description": "Valid Description", "status": "active"},
    ...
    response = client.get(
        f"/api/v1/projects/{wrong_proj_id}/tasks/{task['id']}", headers=user["headers"]
    )
...
def test_api1_bola_user_a_cannot_access_comments_of_other_project_task(client, create_user):
    ...
    p1_resp = client.post(
        "/api/v1/projects",
        json={"name": "Project Beta", "description": "Valid Description", "status": "active"},
        headers=user["headers"],
    )
    assert p1_resp.status_code == 201
    p1 = p1_resp.json()
    ...
    response = client.get(
        f"/api/v1/projects/{p1['id']}/tasks/{invalid_task_id}/comments", headers=user["headers"]
    )
    assert response.status_code in [403, 404]
...
def test_api2_broken_auth_invalid_credentials_prevent_enumeration(client):
    ...
    bad_login_resp = client.post(
        "/api/v1/auth/login",
        json={"email": "auth_victim@example.com", "password": "WrongPassword999!"},
    )
...
def test_api2_broken_auth_refresh_token_revoked_on_logout(client):
    ...
    # Logout
    logout_resp = client.post("/api/v1/auth/logout", json={"refresh_token": user["refresh_token"]})
    assert logout_resp.status_code == 200

    # Reuse attempt
    reuse_resp = client.post("/api/v1/auth/refresh", json={"refresh_token": user["refresh_token"]})
...
def test_api3_mass_assignment_cannot_elevate_role_to_admin(client):
    ...
    login_resp = client.post(
        "/api/v1/auth/login", json={"email": "hacker@example.com", "password": "Password123!"}
    )
...
def test_api4_unrestricted_resource_consumption_page_size_capped(client, create_user):
    ...
    proj_resp = client.post(
        "/api/v1/projects",
        json={"name": "Resource Project", "description": "Testing limits", "status": "active"},
        headers=user["headers"],
    )
    project_id = proj_resp.json()["id"]

    response = client.get(
        f"/api/v1/projects/{project_id}/tasks?page_size=5000", headers=user["headers"]
    )
