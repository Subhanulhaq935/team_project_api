...
def create_user_refresh_token(db: Session, user_id: int):
    ...
    refresh_token_repository.create_refresh_token(db, user_id, token_hash, expires_at)
...
def refresh_access_token(db: Session, refresh_token: str):
    ...
    active_tokens = refresh_token_repository.get_all_active_refresh_tokens(db)
    ...
    if stored_token.expires_at < now:
        return None
    # Get user
    user = user_repository.get_user_by_id(db, stored_token.user_id)
    ...
    refresh_token_repository.revoke_refresh_token(db, stored_token)

    # Create new access token
    access_token = create_access_token(user.id, user.role)

    # Create new refresh token
    new_refresh_token = create_user_refresh_token(db, user.id)
